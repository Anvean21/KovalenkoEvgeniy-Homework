    ...
    class Program
    {
        static void Main(string[] args)
        {
           var customers = new List<Customer>
           {
            ...
            var idFilter = customers.FirstOrDefault(x => x.Id == id);
            Console.WriteLine(idFilter.Name);

            //Поиск по части имени
            Console.WriteLine("Name - ");
            var userName = Console.ReadLine().ToLower();
            var customersNames = customers.Where(x => x.Name.Contains(userName)).ToList();
        ...
        public static void FilterDate(List<Customer> customers, DateTime from, DateTime to)
        {
            var dates = customers.Where(x => x.RegistrationDate >= from && x.RegistrationDate <= to).ToList();
            if (dates.Any())
            {
        ...
        public static List<Customer> CustomerSort(List<Customer> customers, string property, bool order)
        {
            var type = typeof(Customer);
            var sortProperty = type.GetProperty(property);
           customers = order == true ? customers.OrderBy(p => sortProperty.GetValue(p, null)).ToList() : customers.OrderByDescending(p => sortProperty.GetValue(p, null)).ToList();
            return customers;
        }
    }
}
